...
impl FunboyDatabase {
    ...
    pub async fn get_permissions(&self, user_id: KeySize) -> Result<Permissions, Error> {
        ...
        let mut permissions = HashSet::<Permission>::new();

        for row in rows {
            match row.parse::<Permission>() {
                Ok(permission) => {
                    permissions.insert(permission);
                }
                Err(_) => {
                    eprintln!("unknown permission {} in database", row);
                }
            }
        }
...
pub mod test {
    ...
    pub async fn create_debug_db(pool: PgPool) -> Result<FunboyDatabase, sqlx::Error> {
        FunboyDatabase::migrate(&pool).await?;
        let debug_db = FunboyDatabase::new(Arc::new(pool));
