...
impl FunboyDatabaseOld {
    ...
    pub fn from_path(path: &str) -> rusqlite::Result<FunboyDatabaseOld> {
        let db = Connection::open(path)?;

        Self::initialize_db(&db)?;

        Ok(FunboyDatabaseOld { db })
    }
    ...
    fn get_template_replacements(old: &str, new: &str) -> Vec<TemplateReplacement> {
        ...
        for header in interpolator::TEMPLATE_HEADERS {
...
mod tests {
    ...
    fn get_inside_empty_database() {
        let db = FunboyDatabaseOld::from_path("test1.db").unwrap();
    ...
    fn insert_new_templates_with_subtitutions() {
        let mut db = FunboyDatabaseOld::from_path("test2.db").unwrap();
    ...
    fn insert_only_template() {
        let mut db = FunboyDatabaseOld::from_path("test4.db").unwrap();
    ...
    fn remove_substitutes() {
        let mut db = FunboyDatabaseOld::from_path("test5.db").unwrap();
    ...
    #[test]
    fn remove_template() {
        let mut db = FunboyDatabaseOld::from_path("test6.db").unwrap();
    ...
    #[test]
    fn rename_template() {
        let mut db = FunboyDatabaseOld::from_path("test7.db").unwrap();
    ...
    fn insert_substitutes_with_same_name() {
        let mut db = FunboyDatabaseOld::from_path("test8.db").unwrap();
    ...
    fn insert_substitutes_with_same_name_with_same_template() {
        let mut db = FunboyDatabaseOld::from_path("test9.db").unwrap();
